...
const pagesArray = ["https://apistatus.chorus.co.nz", "https://status.blazemeter.com", "https://status.internetnz.nz"];
    ...
    fetch(pagesArray[i] + "/api/v2/summary.json")//calls the Atlassian API associated with that status page
        ...
        .then((data) => {
            ...
            if(componentList != 0) {
                ...
                document.getElementById(data.page.id).innerHTML = `<div class="statusHoverDefault"><p><div class="trafficLight ${data.status.indicator}"></div>&nbsp;&nbsp;<a href="${data.page.url}" target="_blank">${data.page.name} Status</a> - ${data.status.description}</p><span class="statusHoverActive">${completeList}</span>`;
            }
        })
